...
RoutePlanner::RoutePlanner(RouteModel &model, float start_x, float start_y, float end_x, float end_y): m_Model(model) {
    ...
    this->start_node = &m_Model.FindClosestNode(start_x, start_y); // start_node is a pointer
    this->end_node = &m_Model.FindClosestNode(end_x, end_y); // end_node is also a pointer
}
...
RouteModel::Node *RoutePlanner::NextNode() {
    // open_list -> std::vector<RouteModel::Node*>
    // n1 and n2 are pointers
    std::sort(this->open_list.begin(), this->open_list.end(), [](const auto &n1, const auto &n2){
        return (n1->g_value + n1->h_value) > (n2->g_value + n2->h_value);
    });

    RouteModel::Node *node_with_lowest_sum = this->open_list.back();
